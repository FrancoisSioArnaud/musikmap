...
class PointsAndReactionDoubleActionTests(FlowboxAPITestCase):
    def test_double_reveal_keeps_one_discovery_and_one_debit(self):
        ...
        deposit = self.make_deposit(user=self.make_user(username="owner-duprev"), song=self.make_song(public_key="duprev-song"), box=box)
    ...
    def test_double_pin_creates_single_active_pin_and_single_debit(self):
        ...
        payload = {"boxSlug": box.url, "duration_minutes": 10, "option": self.track_option(track_id="duppin-track", title="Dup pin track")}
    ...
    def test_reaction_change_keeps_single_row(self):
        ...
        self.client.post(reverse("reactions"), {"dep_public_key": deposit.public_key, "emoji_id": emoji_b.id}, format="json")
...
class CommentAndShareDoubleActionTests(FlowboxAPITestCase):
    ...
    def test_double_comment_report_creates_single_report(self):
        ...
        comment = Comment.objects.create(client=self.client_entity, deposit=self.deposit, user=comment_author, text="À signaler", normalized_text="à signaler")
        reporter = self.auth(self.make_user(username="reporter", points=0))

        first = self.client.post(reverse("comments-report", kwargs={"comment_id": comment.id}), {"reason": "spam"}, format="json")
        second = self.client.post(reverse("comments-report", kwargs={"comment_id": comment.id}), {"reason": "spam"}, format="json")
    ...
    def test_double_comment_delete_is_idempotent_and_does_not_duplicate_decision(self):
        ...
        comment = Comment.objects.create(client=self.client_entity, deposit=self.deposit, user=author, text="À supprimer", normalized_text="à supprimer")
        ...
        self.assertEqual(
            CommentModerationDecision.objects.filter(comment=comment, acted_by=author, decision_code="delete_by_author").count(),
            1,
        )
    ...
    def test_double_share_link_create_reuses_same_link(self):
        ...
        first = self.client.post(reverse("share-link-create"), {"dep_public_key": self.deposit.public_key}, format="json")
        second = self.client.post(reverse("share-link-create"), {"dep_public_key": self.deposit.public_key}, format="json")
