...
export default function PlayDrawer({ open, song, onClose, onSongResolved, children }) {
  ...
  const actions = [
    { key: "spotify", label: "Spotify", onClick: () => resolveAndOpenProvider("spotify"), icon: logoByPlatform.spotify },
    { key: "deezer", label: "Deezer", onClick: () => resolveAndOpenProvider("deezer"), icon: logoByPlatform.deezer },
    { key: "youtube", label: "YouTube", onClick: openYouTubeSearch, icon: logoByPlatform.youtube },
    { key: "copy", label: "Copier le nom de la chanson", onClick: copySongText },
  ];
