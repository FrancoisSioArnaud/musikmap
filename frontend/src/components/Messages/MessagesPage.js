...
export default function MessagesPage() {
  ...
  return <Box sx={{ p: 2, pb: 8 }}>
{loading ? <CircularProgress />
  : 
  <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "320px 1fr" }, gap: 2 }}><Box><Tabs value={activeTab} onChange={(_, n) => setActiveTab(n)}><Tab value="conversations" label="Conversations" /><Tab value="invitations" label="Invitations" /></Tabs>
  <List>{displayedItems.map((item) => <MessageRow key={`${activeTab}-${item.id}`} item={item} active={normalize(item?.other_user?.username) === normalize(selectedThreadUsername)} onClick={() => handleSelectItem(item)} sx={{ justifyContent: "space-between" }} />)}</List>
