...
def serialize_provider_connection(connection: Optional[UserProviderConnection], provider_code: Optional[str] = None):
    ...
    return {
        ...
        "scopes": scopes,
        "can_recent_plays": connection.provider_code in {"spotify", "deezer"} and bool(connection.is_active and connection.access_token),
    }
...
def serialize_provider_connections_for_user(user: Optional[CustomUser]):
    if not user or not getattr(user, "pk", None):
        return {
            "spotify": serialize_provider_connection(None),
            "deezer": serialize_provider_connection(None),
        }
